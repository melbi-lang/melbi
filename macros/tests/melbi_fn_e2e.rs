...
fn string_unicode() {
    ...
    assert_eq!(
        ctx.call_ok(DeclUpper::new(ctx.type_mgr), &[ctx.str("Γεια σας κόσμε")])
            .as_str()
            .unwrap(),
        "Γεια σας κόσμε"
    );
...
#[test]
fn no_context_result_returns_float_success() {
    let arena = Bump::new();
    let ctx = TestCtx::new(&arena);
    assert_eq!(
        ctx.call_ok(
...
#[test]
fn no_context_result_returns_float_error() {
    let arena = Bump::new();
    let ctx = TestCtx::new(&arena);
    let err = ctx
        .call(
...
#[test]
fn derived_name_with_underscores() {
    let arena = Bump::new();
    let ctx = TestCtx::new(&arena);
