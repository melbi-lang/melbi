...
impl<B: TreeBuilder, C> Visit<B, C> for Tree<B> where TreeNode<B>: Visit<B, C> {
    type Output = <TreeNode<B> as Visit<B, C>>::Output;
    fn visit(&self, ctx: &mut C) -> Self::Output { self.node().visit(ctx) }
}

#[derive(Default)] pub struct Count(usize);
// Two blanket impls, one per builder trait -- do they overlap?
impl<B: ExprBuilder> Visit<B, Count> for TreeNode<B> {
    type Output = ();
    fn visit(&self, ctx: &mut Count) {
        ctx.0 += 1;
        match self.kind() { ExprKind::Neg(i) => i.visit(ctx), ExprKind::Lit(l) => l.visit(ctx) }
    }
}
impl<B: LitBuilder> Visit<B, Count> for TreeNode<B> {
    type Output = ();
    fn visit(&self, ctx: &mut Count) {
        ctx.0 += 1;
        match self.kind() { LitKind::Int(_) => {}, LitKind::Suffixed(_, s) => s.visit(ctx) }
    }
}
